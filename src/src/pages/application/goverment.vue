...
import { applicationAbout, applicationBusiness, applicationList, applicationPersonnel } from "@/router/routes/application";
...
onResultTree(() => {
  ...
  const applicationGoverment = applicationTree.value?.find(item => item.name === APPLICATION_STEPS.GOVERMENT);

  const applicationDataValue = cloneDeep(applicationGoverment?.children) ?? [];
  ...
  applicationProjects.value = cloneDeep(applicationGoverment?.projects) ?? [];
...
function toNextStepPage() {
  void router.push({ name: applicationBusiness.name, params: { id: id.toString(), type } });
}
...
<template>
  <div class="container">
    <ApplicationHeader
      ...
      :step="APPLICATION_STEPS.GOVERMENT"
    />
