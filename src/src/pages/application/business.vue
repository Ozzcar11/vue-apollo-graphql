...
import { applicationAbout, applicationGoverment, applicationList } from "@/router/routes/application";
...
const type = route.params.type ?? "edit";
...
function toNextStepPage() {
  void router.push({ name: applicationList.name, params: { id: id.toString() } });
}
...
function statusHandler() {
  if (application.value?.status !== APPLICATION_STATUS.PROCESSED) {
    void updateApplication({
      userApplication: {
        id,
        status: APPLICATION_STATUS.PROCESSED,
      },
    });
  }
}
...
<template>
  <div class="container">
    <ApplicationHeader
      ...
      :step="APPLICATION_STEPS.BUISINESS"
    />
